...
{
    public class DefaultConsensusCryptoBackend<T> : IConsensusCryptoBackend<T>
        where T : HashAlgorithm
    {
        private ECDomainParameters _eCParams;

        public DefaultConsensusCryptoBackend(ImmutableArray<byte>? suiteBytes=null)
        {
            X9ECParameters ps = ECNamedCurveTable.GetByName("secp256k1");
            _eCParams = new ECDomainParameters(ps.Curve, ps.G, ps.N, ps.H);
            SuiteBytes = suiteBytes ?? new byte[] { 1 }.ToImmutableArray();
        }
